...
class FusionCore	{
	...
	public function admin_enqueue_scripts_styles($hook_suffix) {
		...
		if ( ($hook_suffix == 'post.php' || $hook_suffix == 'post-new.php') && ( (post_type_exists('notification') && $post->post_type == 'notification') || (!empty($fsn_post_types) && is_array($fsn_post_types) && in_array($post->post_type, $fsn_post_types)) ) ) {
			//bootstrap
			wp_enqueue_script( 'bootstrap_admin', plugin_dir_url( __FILE__ ) . 'includes/bootstrap/admin/js/bootstrap.bundle.min.js', false, '4.3.0', true );
			//jQuery UI
	...
	public function front_enqueue_scripts_styles() {
		...
		if (!empty($bootstrap_enable)) {
			wp_enqueue_script( 'bootstrap', plugin_dir_url( __FILE__ ) . 'includes/bootstrap/front/js/bootstrap.bundle.min.js', false, '4.3.0', true );
			wp_enqueue_style( 'bootstrap', plugin_dir_url( __FILE__ ) . 'includes/bootstrap/front/css/bootstrap.min.css', false, '4.3.0' );
			wp_enqueue_style( 'bootstrap', plugin_dir_url( __FILE__ ) . 'includes/bootstrap/front/css/bootstrap-reboot.min.css', false, '4.3.0' );
			wp_enqueue_style( 'bootstrap', plugin_dir_url( __FILE__ ) . 'includes/bootstrap/front/css/bootstrap-grid.min.css', false, '4.3.0' );
	...
	public function column_shortcode($atts, $content = null) {
		...
		if ( is_admin() && (!defined('DOING_AJAX') || !DOING_AJAX || (!empty($ajax_action) && $ajax_action == 'load_template' || $ajax_action == 'components_modal')) ) {
			...
			$output = '';
			$output .= '<div class="col-md-'. esc_attr($width) . (!empty($offset) ? ' offset-md-'. esc_attr($offset) : '') .'"'. $shortcode_atts_data .'>';
			...
			$output .= '<div class="col-md-'. esc_attr($width) . (!empty($offset) ? ' offset-md-'. esc_attr($offset) : '') .'"><div class="fsn-column-inner '. fsn_style_params_class($atts) . (!empty($classes) ? ' '. esc_attr($classes) : '') .'"'. (!empty($style) ? ' style="'. esc_attr($style) .'"' : '') .'>'. do_shortcode($content) .'</div></div>';
