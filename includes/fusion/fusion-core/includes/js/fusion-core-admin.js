...
function fsnInitUIevents(instance) {
	...
	instance.on('click', '.fsn-add-col', function() {
		...
		if (nextItem.length > 0) {
			...
			var matchedClass = classes.match(/offset-md-[0-9]+/);
	...
	instance.on('click', '.delete-col', function(e) {
		...
		if (nextItem.length > 0) {
			...
			var itemColOffsetClass = itemClasses.match(/offset-md-[0-9]+/);
			...
			nextItem.addClass('offset-md-'+ newOffset);
...
function fsnAddColFields(instance) {
	...
	//in offset areas
	var colsWithOffset = instance.find('div[class*="offset-md"]');
	...
	rows.each(function() {
		...
		cols.each(function() {
			...
			var itemColClass = itemClasses.match(/col-md-[0-9]+/);
	...
	instance.find('.row').sortable({
		...
		stop: function(event, ui) {
			...
			if (offset >= 0) {
				...
				if (offset > 0) {
					currentItem.addClass('offset-md-'+ offset);
			...
			if (nextItem.length != 0) {
				...
				var nextItemMatchedClass = nextItemClasses.match(/offset-md-[0-9]+/);
				...
				if (nextItemNewOffset > 0) {
					nextItem.addClass('offset-md-'+ nextItemNewOffset);
		...
		stop: function(event, ui) {
			...
			if (newLeft == originalLeft) {
				...
				if (colDifference > 0) {
					...
					//add new offset class and data attr
					if (newOffset > 0) {
						currentItem.addClass('offset-md-'+ newOffset);
